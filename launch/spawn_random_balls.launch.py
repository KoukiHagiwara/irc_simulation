...
def generate_launch_description():
    ...
    # モデルファイルのパス設定
    model_base_path = os.path.join(get_package_share_directory('irc_simulation'),'models')
    color_models = {
    ...
    areas = []
    for (x0, y0, x1, y1) in base_areas:
        areas.append((x0 + x_offset, y0 + y_offset, x1 + x_offset, y1 + y_offset))
